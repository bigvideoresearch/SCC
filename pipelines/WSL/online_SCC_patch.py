...
@runner.patch_pipeline('OnlineSelfContainedConfPipeline')
class OnlineSelfContainedConfPipeline(ReWeightLearnPipeline):

    def train_epoch(self):
        network = self.network_dict['main']
        teacher_network = self.network_dict['teacher']
        self.train_dataiter = iter(self.loader_dict['train'])
        transform_batch = self.transform_batch_dict['train']
        transform_batch_teacher = self.transform_batch_dict['valid']
        ...
        for self.train_step in range(1, len(self.train_dataiter) + 1):
            with recorder.record_time('batch'):
                ...
                with torch.no_grad():
                    ...
                    with recorder.record_time('transform_teacher_batch'):
                        batch_teacher = transform_batch_teacher(batch)
                    with recorder.record_time('teacher_net_forward'):
                        logit_teacher = teacher_network(batch_teacher['aux_data'])
                    score = torch.nn.functional.sigmoid(logit_teacher)
                    # Get pseudo labels
                    ...
                    conf = torch.sum(batch_teacher['soft_label'] * score, dim=1)
                    if self.config.SCC_setting.conf_power != 1:     # confidence calibration
                        batch['conf_score'] = ScalingConf(conf, self.config.SCC_setting.conf_power)
                    else:
                        batch['conf_score'] = conf
                ...
                #######################################################################################################
                with recorder.record_time('backprop'):
    ...
    def get_loader(self):
        ...
        for name in self.config.data.names:
            ...
            transform_aux_image = TransformCompose([
                getattr(transforms_image, name)(**kwargs)
                for name, kwargs in conf.transform_aux_image.items()
            ])
            ...
            imglist = runner.get_imglist(conf.imglist_type, conf.imglist_path)
            reader = runner.get_filereader(conf.reader)
            ...
            if conf.dataset.name == 'OnlineLabelerDataset':
                lib = datasets
                ...
                dataset = lib(
                    ...
                    transform_aux_image=transform_aux_image,
