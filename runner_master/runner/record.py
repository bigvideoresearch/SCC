...
class RecordManager:
    ...
    def __init__(self, window_size=None, summary_mode=None, sync_cuda=False):
    ...
    def record_value(self, key, value, window_size=None, summary_mode=None, group='default'):
        ...
        assert type(record) is Record, \
            'renew record [{}] in group [{}] with different summary_mode is not allowed'.format(key, group)
        assert record.window_size == window_size, \
            'renew record [{}] in group [{}] with different window_size is not allowed'.format(key, group)
        record.record_value(value)

    def record_tick(self, key):
        self.ticks[key] = time.time()

    def record_tock(self, key, window_size, summary_mode, group='time'):
    ...
    @contextmanager
    def record_time(self, key, window_size=None, summary_mode=None, group='default'):
        if window_size is None:
            window_size = self.window_size
        if summary_mode is None:
            summary_mode = self.summary_mode
    ...
    def keys(self, group='default', group_must_exist=True):
        if group not in self.groups:
            if group_must_exist:
                raise KeyError('group %s not exists' % repr(group))
            else:
    ...
    def values(self, group='default', group_must_exist=True):
        if group not in self.groups:
            if group_must_exist:
                raise KeyError('group %s not exists' % repr(group))
            else:
    ...
    def items(self, group='default', group_must_exist=True):
        if group not in self.groups:
            if group_must_exist:
                raise KeyError('group %s not exists' % repr(group))
            else:
    ...
    def clear_records(self, group='default', group_must_exist=True):
        if group not in self.groups:
            if group_must_exist:
                raise KeyError('group %s not exists' % repr(group))
            else:
