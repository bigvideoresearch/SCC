__all__ = ['SoftCrossEntropyFunction', 'soft_cross_entropy', 'SoftCrossEntropyLoss']
    ...
    def forward(ctx, logit, label, weight=None):
        ...
        if weight is None:
            ...
            if weight.size() != (logit.size(-1),):
                raise ValueError('since logit.size() = {}, weight.size() should be ({},), but got {}'.format(
                    logit.size(), logit.size(-1), weight.size(),
                ))
            size = [1] * label.dim()
            size[-1] = label.size(-1)
