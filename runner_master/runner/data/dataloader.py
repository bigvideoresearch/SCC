...
import os
import atexit
from ._six import string_classes, int_classes
...
def _worker_loop(index_queue, data_queue, done_event, seed, init_fn, worker_id):
    ...
    try:
        ...
        while watchdog.is_alive():
            ...
            # r[0] being None indicates new dataset and collate_fn
            if r[0] is None:
                _, dataset, collate_fn = r
                continue
...
class _DataLoaderIter(object):
    ...
    def __new__(cls, num_workers, pin_memory, timeout, worker_init_fn, multiton_id):
        key = (num_workers, pin_memory, timeout, worker_init_fn, multiton_id)
    ...
    def __init__(self, num_workers, pin_memory, timeout, worker_init_fn, multiton_id):
        ...
        if self.num_workers > 0:
            ...
            for i in range(self.num_workers):
                ...
                w = multiprocessing.Process(
                    target=_worker_loop,
                    args=(index_queue, self.worker_result_queue, self.done_event,
    ...
    def __len__(self):
        if not hasattr(self, 'batch_sampler'):
            raise RuntimeError('batch_sampler not set for _DataLoaderIter instances.')
    ...
    def reset(self, dataset, collate_fn, batch_sampler):
        if not hasattr(self, 'sample_iter'):
            depleted = True
        elif self.num_workers == 0:
            depleted = next(self.sample_iter, None) is None
        else:
            depleted = next(self.sample_iter, None) is None and self.batches_outstanding == 0
        if not depleted:
            raise RuntimeError('_DataLoaderIter not depleted yet, cannot be reset!')

        # reset ne wdata resources
        ...
        if self.num_workers > 0:
            # reset multiprocessing indices and counters
            self.worker_queue_idx = 0
            self.batches_outstanding = 0
            self.send_idx = 0
            self.rcvd_idx = 0
            ...
            # send new dataset and collate_fn to each workers
            for worker_queue_idx in range(self.num_workers):
                self.index_queues[worker_queue_idx].put((None, dataset, collate_fn))
...
class DataLoader(object):
    ...
    def __init__(self, dataset, batch_size=1, shuffle=False, sampler=None, batch_sampler=None,
                 ...
                 timeout=0, worker_init_fn=None, multiton_id=0):
        self.dataset = dataset
        ...
        self.batch_sampler = batch_sampler
        self.dataiter = _DataLoaderIter(self.num_workers, self.pin_memory, self.timeout,
                                        self.worker_init_fn, self.multiton_id)
        self.__initialized = True

    def __setattr__(self, attr, val):
        if self.__initialized and attr in ('batch_size', 'sampler', 'drop_last', 'dataiter'):
    ...
    def __iter__(self):
        self.dataiter.reset(self.dataset, self.collate_fn, self.batch_sampler)
