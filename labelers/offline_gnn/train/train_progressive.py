...
from labelers.offline_gnn import model
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_df_path', type=str, default='', help="Path towards dataframe")

    # seed selection
    parser.add_argument('--threshold_coi', type=float, default=0.7, help='Threshold of seed selection')
    ...
    parser.add_argument('--train_score_path', type=str, default='', help='SGC training score path.')
    ...
    parser.add_argument('--train_fast', type=str, default='False', help='Train fast but potential MemoryErr.')
    ...
    ###################################################################################################################
    #                                      Select seed based on model scores                                          #
    ###################################################################################################################
    df_file_path = args.train_df_path
    train_imglist_path = args.train_imglist_path
    if os.path.exists(df_file_path):
        df = pd.read_pickle(df_file_path)
    else:
        df = utils.create_df(train_imglist_path, df_file_path)
    class_num = np.unique(np.array(df['y']))

    pred_logits = np.load(args.train_score_path)
    threshold_coi = args.threshold_coi

    new_seed_label = np.zeros(pred_logits.shape)
    new_seed_label[np.where(pred_logits > threshold_coi)[0], np.where(pred_logits > threshold_coi)[1]] = 1
    new_seed_idx = np.where(np.sum(new_seed_label, axis=1) > 0)[0].astype(np.int32)
    new_seed_label = new_seed_label[new_seed_idx]
    class_freq = np.sum(new_seed_label, axis=0)
    class_weight = 1 / class_freq
    sample_weight = np.dot(new_seed_label / np.sum(new_seed_label, axis=1).reshape(-1, 1), class_weight)
    sample_weight *= 1 / sample_weight.mean()
    print('Number of seeds: {}.'.format(len(new_seed_label)), flush=True)
    ...
    train_fast = (args.train_fast == 'True')
    ##################################################################################################################
    ...
    seed_softlabel = torch.FloatTensor(new_seed_label)
    seed_reweight_list = torch.FloatTensor(sample_weight)

    if graph_method != 'cpu':
        if train_fast:
            sgc_model, train_time, acc_val = model.train_regression_with_softlabel_fast(sgc_model, features[new_seed_idx],
                                                                                   seed_softlabel, features[val_idx],
                                                                                   label[val_idx], seed_reweight_list,
                                                                                   epochs_num, weight_decay, lr,
                                                                                   loss_type, batch_size, dropout)
        else:
            sgc_model, train_time, acc_val = model.train_regression_with_softlabel(sgc_model, features[new_seed_idx],
                                                                                   seed_softlabel, features[val_idx],
                                                                                   label[val_idx], seed_reweight_list,
                                                                                   epochs_num, weight_decay, lr,
                                                                                   loss_type, batch_size, dropout)
    else:
        sgc_model, train_time, acc_val = model.train_regression_with_softlabel_nocuda(sgc_model, features[new_seed_idx],
                                                                                      seed_softlabel, features[val_idx],
                                                                                      label[val_idx], seed_reweight_list,
                                                                                      epochs_num, weight_decay, lr,
                                                                                      loss_type, batch_size, dropout)
