...
def active_sce_sampling(sampler, logits, indexes, image_names):
    ...
    f_autolabeled.close()
    if sampler.use_al.use:
        print('active_sce_sampling done, with active mining, \
              need to human-labeled num: {}'
              .format(len(active_set)))
    else:
        print('active_sce_sampling done, without active mining')
