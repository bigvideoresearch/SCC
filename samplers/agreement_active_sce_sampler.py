...
def agreement_active_sce_sampling(sampler, logits_dict, indexes, image_names):
    """Mining the positive samples with high confidence of top-k
       of each class while split the score to active mining hard
       samples with two models

    Args:
        sampler (dict): the configuration of the agreement sampler
    ...
    hardsample_dictionary = collections.defaultdict(list)
    hard = -1
    ...
    for i in tqdm(range(len(indexes))):
        ...
        if len(common_idxs) == 0:
            hardsample_dictionary[str(hard)].append(image_names[indexes[i]])
        else:
    ...
    if sampler.use_al.use:
        print(
            'agreement_activte_sce_sampling done, \
             with active mining, need to human-labeled num: {}'
            .format(len(active_set)))
    else:
        print('agreement_activte_sce_sampling done, without active mining')
